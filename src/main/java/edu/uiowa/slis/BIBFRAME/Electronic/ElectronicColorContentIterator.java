...
public class ElectronicColorContentIterator extends edu.uiowa.slis.BIBFRAME.TagLibSupport {
	static ElectronicColorContentIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(ElectronicColorContentIterator.class);

	String subjectURI = null;
	String colorContent = null;
	ResultSet rs = null;
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			rs = getResultSet(Prefix_1_4+"SELECT ?s where { <" + subjectURI + "> <http://bib.ld4l.org/ontology/legacy/colorContent> ?s } ");
			if(rs.hasNext()) {
				QuerySolution sol = rs.nextSolution();
				colorContent = sol.get("?s").toString();
