...
public class ElectronicFormatOfMusicIterator extends edu.uiowa.slis.BIBFRAME.TagLibSupport {
	static ElectronicFormatOfMusicIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(ElectronicFormatOfMusicIterator.class);

	String subjectURI = null;
	String formatOfMusic = null;
	ResultSet rs = null;
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			rs = getResultSet(Prefix_1_4+"SELECT ?s where { <" + subjectURI + "> <http://bib.ld4l.org/ontology/legacy/formatOfMusic> ?s } ");
			if(rs.hasNext()) {
				QuerySolution sol = rs.nextSolution();
				formatOfMusic = sol.get("?s").toString();
