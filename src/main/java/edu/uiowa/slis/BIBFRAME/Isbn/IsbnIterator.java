...
public class IsbnIterator extends edu.uiowa.slis.BIBFRAME.TagLibSupport {
	static IsbnIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(IsbnIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			rs = getResultSet(prefix+
					" SELECT ?s ?lab where { "+
					"  ?s rdf:type <http://bib.ld4l.org/ontology/Isbn> . "+
			...
			log.error("Exception raised in IsbnIterator doStartTag", e);
			clearServiceState();
			freeConnection();
			throw new JspTagException("Exception raised in IsbnIterator doStartTag");
		}
	...
	public int doAfterBody() throws JspException {
		try {
			...
			log.error("Exception raised in IsbnIterator doAfterBody", e);
			clearServiceState();
			freeConnection();
			throw new JspTagException("Exception raised in IsbnIterator doAfterBody");
		}
	...
	public int doEndTag() throws JspException {
		...
		try {
			...
			log.error("Exception raised in Isbn doEndTag", e);
			throw new JspTagException("Exception raised in Isbn doEndTag");
		} finally {
			clearServiceState();
			freeConnection();
		}
