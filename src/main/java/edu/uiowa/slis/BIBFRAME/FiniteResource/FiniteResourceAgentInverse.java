package edu.uiowa.slis.BIBFRAME.FiniteResource;

import org.apache.commons.logging.Log;
import org.apache.commons.logging.LogFactory;

import javax.servlet.jsp.JspException;
import javax.servlet.jsp.JspTagException;

@SuppressWarnings("serial")
public class FiniteResourceAgentInverse extends edu.uiowa.slis.BIBFRAME.TagLibSupport {
	static FiniteResourceAgentInverse currentInstance = null;
	private static final Log log = LogFactory.getLog(FiniteResourceAgentInverse.class);

	// object property

	public int doStartTag() throws JspException {
		try {
			FiniteResourceAgentInverseIterator theFiniteResourceAgentInverseIterator = (FiniteResourceAgentInverseIterator)findAncestorWithClass(this, FiniteResourceAgentInverseIterator.class);
			pageContext.getOut().print(theFiniteResourceAgentInverseIterator.getAgentInverse());
		} catch (Exception e) {
			log.error("Can't find enclosing FiniteResource for agent tag ", e);
			throw new JspTagException("Error: Can't find enclosing FiniteResource for agent tag ");
		}
		return SKIP_BODY;
	}
}

