...
public class Content extends edu.uiowa.slis.BIBFRAME.TagLibSupport {
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			if (theContentIterator != null) {
				subjectURI = theContentIterator.getSubjectURI();
				label = theContentIterator.getLabel();
			}

			if (theContentIterator == null && subjectURI == null) {
			...
			log.error("Exception raised in Content doStartTag", e);
			throw new JspTagException("Exception raised in Content doStartTag");
		} finally {
			freeConnection();
		}
