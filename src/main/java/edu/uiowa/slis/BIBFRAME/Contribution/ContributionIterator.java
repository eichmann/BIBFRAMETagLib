...
public class ContributionIterator extends edu.uiowa.slis.BIBFRAME.TagLibSupport {
	static ContributionIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(ContributionIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			rs = getResultSet(prefix+
					" SELECT ?s ?lab where { "+
					"  ?s rdf:type <http://bib.ld4l.org/ontology/Contribution> . "+
			...
			log.error("Exception raised in ContributionIterator doStartTag", e);
			clearServiceState();
			freeConnection();
			throw new JspTagException("Exception raised in ContributionIterator doStartTag");
		}
	...
	public int doAfterBody() throws JspException {
		try {
			...
			log.error("Exception raised in ContributionIterator doAfterBody", e);
			clearServiceState();
			freeConnection();
			throw new JspTagException("Exception raised in ContributionIterator doAfterBody");
		}
