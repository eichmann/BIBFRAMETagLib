...
public class FingerprintIterator extends edu.uiowa.slis.BIBFRAME.TagLibSupport {
	static FingerprintIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(FingerprintIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			rs = getResultSet(prefix+
					" SELECT ?s ?lab where { "+
					"  ?s rdf:type <http://bib.ld4l.org/ontology/Fingerprint> . "+
			...
			log.error("Exception raised in FingerprintIterator doStartTag", e);
			clearServiceState();
			freeConnection();
			throw new JspTagException("Exception raised in FingerprintIterator doStartTag");
		}
	...
	public int doAfterBody() throws JspException {
		try {
			...
			log.error("Exception raised in FingerprintIterator doAfterBody", e);
			clearServiceState();
			freeConnection();
			throw new JspTagException("Exception raised in FingerprintIterator doAfterBody");
		}
