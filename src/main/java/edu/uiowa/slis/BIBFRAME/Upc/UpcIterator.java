...
public class UpcIterator extends edu.uiowa.slis.BIBFRAME.TagLibSupport {
	static UpcIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(UpcIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			rs = getResultSet(prefix+
					" SELECT ?s ?lab where { "+
					"  ?s rdf:type <http://bib.ld4l.org/ontology/Upc> . "+
			...
			log.error("Exception raised in UpcIterator doStartTag", e);
			clearServiceState();
			freeConnection();
			throw new JspTagException("Exception raised in UpcIterator doStartTag");
		}
	...
	public int doAfterBody() throws JspException {
		try {
			...
			log.error("Exception raised in UpcIterator doAfterBody", e);
			clearServiceState();
			freeConnection();
			throw new JspTagException("Exception raised in UpcIterator doAfterBody");
		}
