...
public class LocationAccompaniedByIterator extends edu.uiowa.slis.BIBFRAME.TagLibSupport {
	static LocationAccompaniedByIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(LocationAccompaniedByIterator.class);
...
//					continue;
			...
			log.error("Exception raised in LocationAccompaniedByIterator doStartTag", e);
			clearServiceState();
			freeConnection();
			throw new JspTagException("Exception raised in LocationAccompaniedByIterator doStartTag");
		}
...
//					continue;
			...
			log.error("Exception raised in LocationAccompaniedByIterator doAfterBody", e);
			clearServiceState();
			freeConnection();
			throw new JspTagException("Exception raised in LocationAccompaniedByIterator doAfterBody");
		}
	...
	public int doEndTag() throws JspException {
		...
		try {
			...
			log.error("Exception raised in LocationAccompaniedBy doEndTag", e);
			throw new JspTagException("Exception raised in LocationAccompaniedBy doEndTag");
		} finally {
			clearServiceState();
			freeConnection();
		}
	...
	public void setClassFilter(String filterString) {
		...
		for (String filterClass : classFilterArray) {
			log.info("adding filterClass " + filterClass + " to LocationAccompaniedByIterator");
