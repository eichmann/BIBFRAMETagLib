package edu.uiowa.slis.BIBFRAME.Dissertation;

import org.apache.commons.logging.Log;
import org.apache.commons.logging.LogFactory;

import javax.servlet.jsp.JspException;
import javax.servlet.jsp.JspTagException;

@SuppressWarnings("serial")
public class DissertationPerformedAt extends edu.uiowa.slis.BIBFRAME.TagLibSupport {
	static DissertationPerformedAt currentInstance = null;
	private static final Log log = LogFactory.getLog(DissertationPerformedAt.class);

	// object property

	public int doStartTag() throws JspException {
		try {
			DissertationPerformedAtIterator theDissertationPerformedAtIterator = (DissertationPerformedAtIterator)findAncestorWithClass(this, DissertationPerformedAtIterator.class);
			pageContext.getOut().print(theDissertationPerformedAtIterator.getPerformedAt());
		} catch (Exception e) {
			log.error("Can't find enclosing Dissertation for performedAt tag ", e);
			throw new JspTagException("Error: Can't find enclosing Dissertation for performedAt tag ");
		}
		return SKIP_BODY;
	}
}

