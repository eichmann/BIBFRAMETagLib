...
public class IssnLIterator extends edu.uiowa.slis.BIBFRAME.TagLibSupport {
	static IssnLIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(IssnLIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			rs = getResultSet(prefix+
					" SELECT ?s ?lab where { "+
					"  ?s rdf:type <http://bib.ld4l.org/ontology/IssnL> . "+
			...
			log.error("Exception raised in IssnLIterator doStartTag", e);
			clearServiceState();
			freeConnection();
			throw new JspTagException("Exception raised in IssnLIterator doStartTag");
		}
	...
	public int doAfterBody() throws JspException {
		try {
			...
			log.error("Exception raised in IssnLIterator doAfterBody", e);
			clearServiceState();
			freeConnection();
			throw new JspTagException("Exception raised in IssnLIterator doAfterBody");
		}
