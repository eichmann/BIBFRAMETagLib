...
public class ResourceIssuedIterator extends edu.uiowa.slis.BIBFRAME.TagLibSupport {
	static ResourceIssuedIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(ResourceIssuedIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			rs = getResultSet(prefix+"SELECT ?s where { <" + subjectURI + "> <http://purl.org/dc/terms/issued> ?s } ");
			if(rs.hasNext()) {
				QuerySolution sol = rs.nextSolution();
				issued = sol.get("?s").toString();
				firstInstance = true;
