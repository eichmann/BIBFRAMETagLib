...
public class ResourcePreferredNamespacePrefixIterator extends edu.uiowa.slis.BIBFRAME.TagLibSupport {
	static ResourcePreferredNamespacePrefixIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(ResourcePreferredNamespacePrefixIterator.class);

	static boolean firstInstance = false;
	static boolean lastInstance = false;

	String subjectURI = null;
	String preferredNamespacePrefix = null;
	ResultSet rs = null;
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			rs = getResultSet(prefix+"SELECT ?s where { <" + subjectURI + "> <http://purl.org/vocab/vann/preferredNamespacePrefix> ?s } ");
			if(rs.hasNext()) {
				QuerySolution sol = rs.nextSolution();
				preferredNamespacePrefix = sol.get("?s").toString();
				firstInstance = true;
