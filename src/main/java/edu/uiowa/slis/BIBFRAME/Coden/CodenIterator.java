...
public class CodenIterator extends edu.uiowa.slis.BIBFRAME.TagLibSupport {
	static CodenIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(CodenIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			rs = getResultSet(prefix+
					" SELECT ?s ?lab where { "+
					"  ?s rdf:type <http://bib.ld4l.org/ontology/Coden> . "+
			...
			log.error("Exception raised in CodenIterator doStartTag", e);
			clearServiceState();
			freeConnection();
			throw new JspTagException("Exception raised in CodenIterator doStartTag");
		}
	...
	public int doAfterBody() throws JspException {
		try {
			...
			log.error("Exception raised in CodenIterator doAfterBody", e);
			clearServiceState();
			freeConnection();
			throw new JspTagException("Exception raised in CodenIterator doAfterBody");
		}
