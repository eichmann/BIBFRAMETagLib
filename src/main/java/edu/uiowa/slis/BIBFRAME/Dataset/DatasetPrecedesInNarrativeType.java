package edu.uiowa.slis.BIBFRAME.Dataset;

import org.apache.commons.logging.Log;
import org.apache.commons.logging.LogFactory;

import javax.servlet.jsp.JspException;
import javax.servlet.jsp.JspTagException;

@SuppressWarnings("serial")
public class DatasetPrecedesInNarrativeType extends edu.uiowa.slis.BIBFRAME.TagLibSupport {
	static DatasetPrecedesInNarrativeType currentInstance = null;
	private static final Log log = LogFactory.getLog(DatasetPrecedesInNarrativeType.class);

	// object property

	public int doStartTag() throws JspException {
		try {
			DatasetPrecedesInNarrativeIterator theDatasetPrecedesInNarrativeIterator = (DatasetPrecedesInNarrativeIterator)findAncestorWithClass(this, DatasetPrecedesInNarrativeIterator.class);
			pageContext.getOut().print(theDatasetPrecedesInNarrativeIterator.getType());
		} catch (Exception e) {
			log.error("Can't find enclosing Dataset for precedesInNarrative tag ", e);
			throw new JspTagException("Error: Can't find enclosing Dataset for precedesInNarrative tag ");
		}
		return SKIP_BODY;
	}
}

