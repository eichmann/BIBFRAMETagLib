...
public class MotivationIterator extends edu.uiowa.slis.BIBFRAME.TagLibSupport {
	static MotivationIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(MotivationIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			rs = getResultSet(prefix+
					" SELECT ?s ?lab where { "+
					"  ?s rdf:type <http://www.w3.org/ns/oa#Motivation> . "+
			...
			log.error("Exception raised in MotivationIterator doStartTag", e);
			clearServiceState();
			freeConnection();
			throw new JspTagException("Exception raised in MotivationIterator doStartTag");
		}
	...
	public int doAfterBody() throws JspException {
		try {
			...
			log.error("Exception raised in MotivationIterator doAfterBody", e);
			clearServiceState();
			freeConnection();
			throw new JspTagException("Exception raised in MotivationIterator doAfterBody");
		}
