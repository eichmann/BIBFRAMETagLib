...
public class TitleElement extends edu.uiowa.slis.BIBFRAME.TagLibSupport {
	static TitleElement currentInstance = null;
	private static final Log log = LogFactory.getLog(TitleElement.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			TitleElementIterator theTitleElementIterator = (TitleElementIterator) findAncestorWithClass(this, TitleElementIterator.class);

			if (theTitleElementIterator != null) {
				subjectURI = theTitleElementIterator.getSubjectURI();
				label = theTitleElementIterator.getLabel();
			}
			...
			if (theTitleElementIterator == null && subjectURI == null) {
			...
			log.error("Exception raised in TitleElement doStartTag", e);
			throw new JspTagException("Exception raised in TitleElement doStartTag");
		} finally {
			freeConnection();
		}
	...
	public int doEndTag() throws JspException {
		...
		try {
			...
			log.error("Exception raised in TitleElement doEndTag", e);
			throw new JspTagException("Exception raised in TitleElement doEndTag");
		} finally {
			clearServiceState();
			freeConnection();
		}
