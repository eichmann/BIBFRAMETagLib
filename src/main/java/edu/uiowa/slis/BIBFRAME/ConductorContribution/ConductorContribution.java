...
public class ConductorContribution extends edu.uiowa.slis.BIBFRAME.TagLibSupport {
	static ConductorContribution currentInstance = null;
	private static final Log log = LogFactory.getLog(ConductorContribution.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			ConductorContributionIterator theConductorContributionIterator = (ConductorContributionIterator) findAncestorWithClass(this, ConductorContributionIterator.class);

			if (theConductorContributionIterator != null) {
				subjectURI = theConductorContributionIterator.getSubjectURI();
				label = theConductorContributionIterator.getLabel();
			}
			...
			if (theConductorContributionIterator == null && subjectURI == null) {
			...
			log.error("Exception raised in ConductorContribution doStartTag", e);
			throw new JspTagException("Exception raised in ConductorContribution doStartTag");
		} finally {
			freeConnection();
		}
	...
	public int doEndTag() throws JspException {
		...
		try {
			...
			log.error("Exception raised in ConductorContribution doEndTag", e);
			throw new JspTagException("Exception raised in ConductorContribution doEndTag");
		} finally {
			clearServiceState();
			freeConnection();
		}
