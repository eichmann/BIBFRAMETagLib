...
public class NbanIterator extends edu.uiowa.slis.BIBFRAME.TagLibSupport {
	static NbanIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(NbanIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			rs = getResultSet(prefix+
					" SELECT ?s ?lab where { "+
					"  ?s rdf:type <http://bib.ld4l.org/ontology/Nban> . "+
			...
			log.error("Exception raised in NbanIterator doStartTag", e);
			clearServiceState();
			freeConnection();
			throw new JspTagException("Exception raised in NbanIterator doStartTag");
		}
	...
	public int doAfterBody() throws JspException {
		try {
			...
			log.error("Exception raised in NbanIterator doAfterBody", e);
			clearServiceState();
			freeConnection();
			throw new JspTagException("Exception raised in NbanIterator doAfterBody");
		}
