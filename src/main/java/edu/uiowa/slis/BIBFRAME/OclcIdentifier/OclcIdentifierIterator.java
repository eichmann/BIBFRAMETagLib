...
public class OclcIdentifierIterator extends edu.uiowa.slis.BIBFRAME.TagLibSupport {
	static OclcIdentifierIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(OclcIdentifierIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			rs = getResultSet(prefix+
					" SELECT ?s ?lab where { "+
					"  ?s rdf:type <http://bib.ld4l.org/ontology/OclcIdentifier> . "+
			...
			log.error("Exception raised in OclcIdentifierIterator doStartTag", e);
			clearServiceState();
			freeConnection();
			throw new JspTagException("Exception raised in OclcIdentifierIterator doStartTag");
		}
	...
	public int doAfterBody() throws JspException {
		try {
			...
			log.error("Exception raised in OclcIdentifierIterator doAfterBody", e);
			clearServiceState();
			freeConnection();
			throw new JspTagException("Exception raised in OclcIdentifierIterator doAfterBody");
		}
