...
public class VideoRecordingNumberIterator extends edu.uiowa.slis.BIBFRAME.TagLibSupport {
	static VideoRecordingNumberIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(VideoRecordingNumberIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			rs = getResultSet(prefix+
					" SELECT ?s ?lab where { "+
					"  ?s rdf:type <http://bib.ld4l.org/ontology/VideoRecordingNumber> . "+
			...
			log.error("Exception raised in VideoRecordingNumberIterator doStartTag", e);
			clearServiceState();
			freeConnection();
			throw new JspTagException("Exception raised in VideoRecordingNumberIterator doStartTag");
		}
	...
	public int doAfterBody() throws JspException {
		try {
			...
			log.error("Exception raised in VideoRecordingNumberIterator doAfterBody", e);
			clearServiceState();
			freeConnection();
			throw new JspTagException("Exception raised in VideoRecordingNumberIterator doAfterBody");
		}
