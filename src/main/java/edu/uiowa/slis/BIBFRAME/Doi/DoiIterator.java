...
public class DoiIterator extends edu.uiowa.slis.BIBFRAME.TagLibSupport {
	static DoiIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(DoiIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			rs = getResultSet(prefix+
					" SELECT ?s ?lab where { "+
					"  ?s rdf:type <http://bib.ld4l.org/ontology/Doi> . "+
			...
			log.error("Exception raised in DoiIterator doStartTag", e);
			clearServiceState();
			freeConnection();
			throw new JspTagException("Exception raised in DoiIterator doStartTag");
		}
	...
	public int doAfterBody() throws JspException {
		try {
			...
			log.error("Exception raised in DoiIterator doAfterBody", e);
			clearServiceState();
			freeConnection();
			throw new JspTagException("Exception raised in DoiIterator doAfterBody");
		}
