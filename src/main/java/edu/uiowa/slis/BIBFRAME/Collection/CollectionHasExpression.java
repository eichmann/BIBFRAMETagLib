package edu.uiowa.slis.BIBFRAME.Collection;

import org.apache.commons.logging.Log;
import org.apache.commons.logging.LogFactory;

import javax.servlet.jsp.JspException;
import javax.servlet.jsp.JspTagException;

@SuppressWarnings("serial")
public class CollectionHasExpression extends edu.uiowa.slis.BIBFRAME.TagLibSupport {
	static CollectionHasExpression currentInstance = null;
	private static final Log log = LogFactory.getLog(CollectionHasExpression.class);

	// object property

	public int doStartTag() throws JspException {
		try {
			CollectionHasExpressionIterator theCollectionHasExpressionIterator = (CollectionHasExpressionIterator)findAncestorWithClass(this, CollectionHasExpressionIterator.class);
			pageContext.getOut().print(theCollectionHasExpressionIterator.getHasExpression());
		} catch (Exception e) {
			log.error("Can't find enclosing Collection for hasExpression tag ", e);
			throw new JspTagException("Error: Can't find enclosing Collection for hasExpression tag ");
		}
		return SKIP_BODY;
	}
}

