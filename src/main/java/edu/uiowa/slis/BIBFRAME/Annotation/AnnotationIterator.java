...
public class AnnotationIterator extends edu.uiowa.slis.BIBFRAME.TagLibSupport {
	static AnnotationIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(AnnotationIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			rs = getResultSet(prefix+
					" SELECT ?s ?lab where { "+
					"  ?s rdf:type <http://www.w3.org/ns/oa#Annotation> . "+
			...
			log.error("Exception raised in AnnotationIterator doStartTag", e);
			clearServiceState();
			freeConnection();
			throw new JspTagException("Exception raised in AnnotationIterator doStartTag");
		}
	...
	public int doAfterBody() throws JspException {
		try {
			...
			log.error("Exception raised in AnnotationIterator doAfterBody", e);
			clearServiceState();
			freeConnection();
			throw new JspTagException("Exception raised in AnnotationIterator doAfterBody");
		}
