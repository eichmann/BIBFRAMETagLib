...
public class LegalDepositNumberIterator extends edu.uiowa.slis.BIBFRAME.TagLibSupport {
	static LegalDepositNumberIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(LegalDepositNumberIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			rs = getResultSet(prefix+
					" SELECT ?s ?lab where { "+
					"  ?s rdf:type <http://bib.ld4l.org/ontology/LegalDepositNumber> . "+
			...
			log.error("Exception raised in LegalDepositNumberIterator doStartTag", e);
			clearServiceState();
			freeConnection();
			throw new JspTagException("Exception raised in LegalDepositNumberIterator doStartTag");
		}
	...
	public int doAfterBody() throws JspException {
		try {
			...
			log.error("Exception raised in LegalDepositNumberIterator doAfterBody", e);
			clearServiceState();
			freeConnection();
			throw new JspTagException("Exception raised in LegalDepositNumberIterator doAfterBody");
		}
