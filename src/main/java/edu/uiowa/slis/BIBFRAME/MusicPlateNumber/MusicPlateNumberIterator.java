...
public class MusicPlateNumberIterator extends edu.uiowa.slis.BIBFRAME.TagLibSupport {
	static MusicPlateNumberIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(MusicPlateNumberIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			rs = getResultSet(prefix+
					" SELECT ?s ?lab where { "+
					"  ?s rdf:type <http://bib.ld4l.org/ontology/MusicPlateNumber> . "+
			...
			log.error("Exception raised in MusicPlateNumberIterator doStartTag", e);
			clearServiceState();
			freeConnection();
			throw new JspTagException("Exception raised in MusicPlateNumberIterator doStartTag");
		}
	...
	public int doAfterBody() throws JspException {
		try {
			...
			log.error("Exception raised in MusicPlateNumberIterator doAfterBody", e);
			clearServiceState();
			freeConnection();
			throw new JspTagException("Exception raised in MusicPlateNumberIterator doAfterBody");
		}
