package edu.uiowa.slis.BIBFRAME.Periodical;

import org.apache.commons.logging.Log;
import org.apache.commons.logging.LogFactory;

import javax.servlet.jsp.JspException;
import javax.servlet.jsp.JspTagException;

@SuppressWarnings("serial")
public class PeriodicalHasPartType extends edu.uiowa.slis.BIBFRAME.TagLibSupport {
	static PeriodicalHasPartType currentInstance = null;
	private static final Log log = LogFactory.getLog(PeriodicalHasPartType.class);

	// object property

	public int doStartTag() throws JspException {
		try {
			PeriodicalHasPartIterator thePeriodicalHasPartIterator = (PeriodicalHasPartIterator)findAncestorWithClass(this, PeriodicalHasPartIterator.class);
			pageContext.getOut().print(thePeriodicalHasPartIterator.getType());
		} catch (Exception e) {
			log.error("Can't find enclosing Periodical for hasPart tag ", e);
			throw new JspTagException("Error: Can't find enclosing Periodical for hasPart tag ");
		}
		return SKIP_BODY;
	}
}

