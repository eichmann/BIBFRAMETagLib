package edu.uiowa.slis.BIBFRAME.Journal;

import org.apache.commons.logging.Log;
import org.apache.commons.logging.LogFactory;

import javax.servlet.jsp.JspException;
import javax.servlet.jsp.JspTagException;

@SuppressWarnings("serial")
public class JournalFollowsType extends edu.uiowa.slis.BIBFRAME.TagLibSupport {
	static JournalFollowsType currentInstance = null;
	private static final Log log = LogFactory.getLog(JournalFollowsType.class);

	// object property

	public int doStartTag() throws JspException {
		try {
			JournalFollowsIterator theJournalFollowsIterator = (JournalFollowsIterator)findAncestorWithClass(this, JournalFollowsIterator.class);
			pageContext.getOut().print(theJournalFollowsIterator.getType());
		} catch (Exception e) {
			log.error("Can't find enclosing Journal for follows tag ", e);
			throw new JspTagException("Error: Can't find enclosing Journal for follows tag ");
		}
		return SKIP_BODY;
	}
}

