...
public class NotatedMovementIterator extends edu.uiowa.slis.BIBFRAME.TagLibSupport {
	static NotatedMovementIterator currentInstance = null;
	private static final Log log = LogFactory.getLog(NotatedMovementIterator.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			rs = getResultSet(Prefix_1_4+
					" SELECT ?s ?l where { "+
						"?s rdfs:label ?l . "+
						"?s rdf:type <http://bib.ld4l.org/ontology/NotatedMovement> . "+
					"} ORDER BY ?l");
			if(rs.hasNext()) {
